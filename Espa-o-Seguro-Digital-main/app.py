...
@app.route('/ajuda')
def ajuda():
    if 'tipo' not in session or session['tipo'] not in ('aluno', 'psicologo'):
        return redirect(url_for('login'))
...
@app.route('/psicologo/agendamentos')
def psicologo_agendamentos():
    if 'tipo' not in session or session['tipo'] != 'psicologo':
        return redirect(url_for('login'))
    return render_template('psicologo_agendamentos.html')

@app.route('/psicologo/historico')
def psicologo_historico():
    if 'tipo' not in session or session['tipo'] != 'psicologo':
        return redirect(url_for('login'))
    return render_template('psicologo_historico.html')
